...
def choose_device(requested="auto"):
    ...
    if requested.lower().startswith("npu"):
        ...
        enable_npu_patch()
        print("[tetris] device: %s" % requested, file=sys.stderr, flush=True)
    else:
        print("[tetris] device: cpu", file=sys.stderr, flush=True)
    return requested
...
def load_agent(model=None, device="auto", backend="torch"):
    ...
    print("[tetris] loading %s ..." % model_path, file=sys.stderr, flush=True)
    agent = load(str(model_path), device=device, backend=backend)
    print("[tetris] loaded on %s" % agent.device, file=sys.stderr, flush=True)
    ...
    if os.environ.get("LAYA_SNAKE_WARMUP", "1").lower() not in ("0", "false", "no"):
        ...
        print("[tetris] warmup done", file=sys.stderr, flush=True)
    return agent
